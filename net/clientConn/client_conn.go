...
func ReadClientMsg(inStream *bufio.Reader) (*ClientMsg, error) {
	...
	msgID := zbyte.ByteToInt(head[:4])
	msgLen := zbyte.ByteToInt(head[4:])
...
func (c *ClientConn) DeliverRecvMsg(call CallBackFunc) {
	for {
		...
		case msg, ok := <-c.recvQueue:
			...
			buf = append(buf, zbyte.IntToByte(res.Tag)...)      // id占4个字节
			buf = append(buf, zbyte.IntToByte(len(res.Msg))...) // 长度占4个字节
